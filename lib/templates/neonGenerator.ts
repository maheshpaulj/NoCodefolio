...
function finalize(data: PortfolioData): PortfolioData {
  return {
    name: data.name || "Your Name",
    bio: data.bio || "Your bio",
    profileImage: data.profileImage || "",
    resumeLink: data.resumeLink || "",
    aboutText: data.aboutText || "About me...",
...
`,
    ...
    "package.json": JSON.stringify({
      name: d.name.toLowerCase().replace(/\s+/g, "-") + "-nocodefolio-neon",
      private: true,
      version: "0.1.0",
